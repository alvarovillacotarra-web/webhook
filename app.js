...
document.addEventListener('keydown', event => { if (event.key === 'Escape' && !teamModal.hidden) closeTeamModal(); if (event.ctrlKey && event.shiftKey && event.key.toLowerCase() === 'w') { event.preventDefault(); if (teamModal.hidden) openTeamModal(); } });
